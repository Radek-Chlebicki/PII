...
let harJson : Archive = JSON.parse(harFile.toString()); 
let storer : Leak[] = []; 
...
// 
function findLeak(aHar : Archive , aStr : string, aStorer : Leak[]){
    let entryNum = 0; 
    aHar.log.entries.forEach((entry: Entry)=>{
...
//
function processRecords(records : Buffer[], harJson : Archive, aStorer : Leak[] ){
...
function printAll(aStorer : any[], outFile : string){
...
fs.createReadStream("./final_hashes_for_lookup.csv")
    ...
    .on("end", ()=>{
        ...
        storer = storer.filter((aLeak)=> {
            ...
            if (SUBDOMAIN !== ""){
                return aLeak["url"].includes(DOMAIN.slice(0,DOMAIN.length -1)) && (!aLeak["url"].includes(SUBDOMAIN))
            }
            else {
                return aLeak["url"].includes(DOMAIN.slice(0,DOMAIN.length -1)) && (aLeak["url"].includes("."+DOMAIN.slice(0,DOMAIN.length -1)))
            }
        }).map((aLeak)=>{
        ...
        let totalCount = storer.length
        let doneCount = 0; 

        let f = (aLeak: Leak) => ((err : NodeJS.ErrnoException | null, ipAddresses : string[])=> {
            if (err){
                // there is no cname
                aLeak["done"] = true
                doneCount +=1
        ...
        // while(! storer.reduce( (acc, currentVal) => {return acc && currentVal["done"] }, true)){
        ...
        function nextPart(){
            ...
            storer = storer
            // at least one cname is a different site
            .filter((aLeak) => {
